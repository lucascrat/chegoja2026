# Recompila e instala APENAS o ChegoJá Motorista no dispositivo indicado
...
Write-Host "Compilando Vite (driver)..."
...
# Guarda copia do APK do motorista para reinstalacoes futuras
Copy-Item "$ANDROID\app\build\outputs\apk\debug\app-debug.apk" "$ROOT\chegoja-motorista-debug.apk" -Force
...
if ([string]::IsNullOrEmpty($DeviceId)) {
    ...
    if ($devices.Count -eq 0) {
        ...
        Write-Host "`nO APK novo foi gerado com sucesso em: $ROOT\chegoja-motorista-debug.apk"
...
$result = & $ADB -s $DeviceId install -r -d "$ROOT\chegoja-motorista-debug.apk" 2>&1
$result
...
if ($hasSuccess) {
    Write-Host "[OK] ChegoJá Motorista instalado em $DeviceId"
    Set-AndroidApp "br.com.client.chegoja" "chegoja"
    exit 0
} else {
